...
Motorbike motorbike3 = new Motorbike(100, color(0, 255, 0));
...
void draw()
{
  ...
  if (keyPressed) {
    ...
    if (gameMode == RACING) {
      ...
      if (motorbike1.finished(460) == true) {
        gameMode = 1;
        winRed += 1;
      }

      if (motorbike2.finished(460) == true) {
        gameMode = 1;
        winBlue += 1;
      }

      if (motorbike3.finished(460) == false) {
        motorbike3.move();
      }

      if (motorbike3.finished(460) == true) {
        gameMode = 1;
        winGreen += 1;
      }
    }
  ...
  if (gameMode == FINISH) {
      background(125);
      text("red:"+winRed, 10, 10);
      text("green:"+winGreen, 10, 40);
      text("blue:"+winBlue, 10, 70);
  }
}
